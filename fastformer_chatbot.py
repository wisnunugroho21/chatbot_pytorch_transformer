...
class MultiHeadAttention(nn.Module):
    def __init__(self, heads: int, d_model: int):
        ...
        self.query_att = nn.Linear(self.d_k, self.d_k)
        self.key_att = nn.Linear(self.d_k, self.d_k)
    ...
    def forward(
        ...
        trans_value = value.view(value.shape[0], self.heads, -1, self.d_k)

        alpha = self.query_att(trans_query) / self.scl_params

        if mask is not None:
            min_type_value = torch.finfo(alpha.dtype).min
            alpha = alpha.masked_fill(mask == 0, min_type_value)

        alpha = F.softmax(alpha, dim=2)
        global_query = (alpha * trans_query).sum(2, keepdim=True)

        phi = global_query * trans_key
        beta = self.key_att(phi) / d_sqrt
        ...
        out = self.out(uhi)  # + query

        return out

        # if mask is not None:
        #     min_type_value  = torch.finfo(query.dtype).min
        #     query  = query.masked_fill(mask == 0, min_type_value)

        # if mask is not None:
        #     min_type_value  = torch.finfo(query.dtype).min
        #     query  = query.masked_fill(mask == 0, min_type_value)

        # alpha   = F.softmax(query, dim = -1)
...
class ConversationDataset(Dataset):
    ...
    def create_src_mask(self, src):
        ...
        src_mask = src_padding_mask.unsqueeze(0).unsqueeze(-1)

        return src_mask

    def create_tgt_mask(self, tgt):
        tgt_padding_mask = tgt != PAD_IDX
        tgt_mask = tgt_padding_mask.unsqueeze(0).unsqueeze(-1)
